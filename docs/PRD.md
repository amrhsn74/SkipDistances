...
|---|---|---|
| **Account Manager** | Owns the client relationship for a set of assigned clients; creates new client accounts, including the market or markets they operate in; **assigns the staff who work on a client** — content lead, creators, client contact; submits campaign briefs on their behalf or converts a client's calendar request into one; connects and manages each client's Instagram account. | A reliable way to get a client's content drafted, approved, and published correctly the first time, and visibility into where every one of their clients' work and performance stands. |
| **Content Creator** | **Originates content in an ongoing conversation with the engine** — working from the client's brief, brand guide and the agency standards — then refines what it produces and submits it for approval; can also refine a draft that arrived from a brief, or regenerate a specific piece using a reference image or document of their own; a client's team can have one or more. | Clear access to just their assigned clients' in-progress work, a conversation with the engine that stays grounded in the right client's rules, and a way to guide it with their own reference material. |
| **Content Lead** | **Prompts the engine to produce a plan or a piece of content, and dispatches what comes out of it to a creator to finish.** When assigned in place of the account manager, also acts as a client's internal reviewer — including pulling back an approval they already gave if something needs a second look. Works across clients. | The same conversation surface the creator has, a way to hand an item to the right creator, and the ability to approve, edit, reject, or reverse an earlier approval on a post, with the reasoning behind each drafted item visible. |
...
| **Agency Admin** | Oversight: the audit trail, the misuse queue, and a cross-client view of where every account stands. Can also assign a client's account manager, reviewer, and creators — a backstop alongside the account manager, who does this routinely. Not involved in day-to-day content work, and holds no drafting or approval power. | A cross-client view of where every account stands, and control over who's assigned where. |
...
- **Client management** — an account manager can create a new client directly in the product, with every field the roster tracks, including which market or markets the client operates in (Egypt, Saudi Arabia, or both). Admin can reassign a client's account manager, reviewer, and creators after the fact.
...
- **Occasion calendar** — the system maintains a calendar of regional and local occasions for each market the client operates in — public holidays, religious observances, retail moments — and surfaces the ones relevant to a campaign so content can be planned and scheduled around them. Dates that move year to year on the Hijri calendar (Ramadan, Eid al-Fitr, Eid al-Adha) are resolved correctly for the current calendar year.
...
- **Chat-led creation** — a content creator, or a content lead, works in an ongoing conversation scoped to one client: they prompt the engine, it answers, and content items come out of the exchange. Every item produced this way goes through the identical guards as one produced from a brief — the client is resolved on the roster, the rules that govern them are retrieved and cited, the draft is compliance-checked, and nothing reaches a channel without internal review and client approval. The conversation is a surface, never a second route to production. Where the exchange has not yet said enough to work from, the engine asks for what is missing rather than guessing — the same completeness rule a brief meets, asked as a question instead of returned as a rejection. A prompt that is not about producing content for that client is refused and raised to the Admin. The whole transcript is retained, so a reviewer can see how a piece came to be, and the Admin can judge conduct across a conversation rather than from a single line.
...
- [ ] Content scheduling correctly reflects upcoming occasions for every market the client operates in, including Hijri-calendar dates for the current year, and a dual-market client's plan schedules each market-specific item on its own market's date without duplicating evergreen content.
...
- **Market** — a real, expandable record rather than a hardcoded value. Egypt and Saudi Arabia are seeded; an account manager picks one or both for each client at creation, so a third market later is a data addition, not a redesign. No sub-national region is tracked. A client operating in both gets one content plan, with each item either market-neutral (produced once — evergreen content, and observances shared across both markets like Ramadan and the two Eids) or tagged to a single market and scheduled against that market's own date (a national day).
...
| §3 Chat-led creation | Client answer — creators and the content lead produce content by prompting the model; transcript retained for misuse review. Guards unchanged: assignment "Produce every campaign through the guarded engine" |
