...
**Off-task generation is refused by the model, not by a keyword list.** The creator-facing engine is a company resource, scoped to the client the conversation is about — not a general assistant. But a keyword filter would block legitimate creative work, so the deterministic pass only ever *allows* — only the generation call's own `on_task` judgment refuses. A false positive here blocks someone's actual job; a false negative costs one wasted call.

Since the engine became conversational (Phase 14), that judgment is made **against the thread, not the turn**. It has to be: "make it shorter" names no client, no brand and no deliverable, and would be refused on its own, yet it is an ordinary thing to say on the fourth turn of a real piece of work. So the check reads the conversation's established client and deliverable and asks whether *this* turn belongs to *that* work. The asymmetry is unchanged — the cheap pass still only ever allows — and the consequence is worth stating plainly: a creator cannot escape the check by splitting an off-task request across turns, because each turn is judged against what the thread is for, not against what the previous turn happened to say.
