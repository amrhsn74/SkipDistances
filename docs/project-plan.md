...
- [ ] `P2.D` — `lib/domain/session.ts`: `createSession` / `resolveSession` / `revokeSession`. Token stored hashed, so a leaked table yields no usable sessions. Unit tests: a revoked or expired session resolves to null; a `disabled` user's session stops resolving immediately.
